...
const fork = require("./fork"),
  ...
  CancelError = require("errno").create("CancelError"),
...
Farm.prototype.stopChild = function(childId) {
  ...
  if (child) {
    child.send({ owner: "farm-enhanced", event: "die" });
    setTimeout(function() {
...
Farm.prototype.forceStopChild = function(childId) {
  let child = this.children[childId],
    i;
  ...
  for (i in child.calls) {
    this.receive({
      idx: i,
      child: childId,
      args: [new CancelError("worker call canceled!")]
    });
  }
  this.stopChild(childId);
};
...
Farm.prototype.end = function(force, callback) {
  this.forceEnd = this.forceEnd || force;
  let complete = true;
